# Qwen2.5-7B 微调记录：AMD MI50 双卡踩坑与优化复盘
...
###阶段二：解决多卡通信死锁 (Deadlock)**现象**: 训练启动后卡在 `ProcessGroupNCCL ... using GPU 0 to perform barrier`，无任何进度，GPU 利用率为 0%。
...
###阶段三：计算效率与算子优化**现象**: IOMMU 关闭后，GPU 利用率飙升至 100%，但速度依然很慢 (**700s+/it**)。
...
* **操作**: 单卡，FP16，Batch=8，关闭 Gradient Checkpointing。
* **性能**: **24s/it** (0.33s/sample)。
* **结论**: 硬件没问题，是 DDP + Gradient Checkpointing + 小 Batch 的组合导致了低效。

---

###阶段四：数值稳定性与最终方案 (The Golden Config)**现象**: FP16 虽然快，但在 Qwen2.5 模型上极其不稳定，必出 NaN。
...
##3. 关键知识库 (Knowledge Base)1. **IOMMU 陷阱**: 在 Intel CPU + AMD GPU 的多卡机器上，必须在 BIOS 或 Grub 中关闭 IOMMU (`intel_iommu=off`)，否则 PCIe P2P 通信会死锁。
2. **MI50 精度支持**: MI50 (Vega20) **不支持 BF16 硬件加速**。强开 BF16 会导致百倍级减速。FP16 速度快但易溢出 (NaN)。FP32 最稳但显存占用大。
3. **Flash Attention**: MI50 对 PyTorch 2.x 的 SDPA (Flash Attention) 支持不佳，建议显式禁用 (`--flash_attn disabled`)，使用 Vanilla Attention 反而兼容性最好。
4. **通信协议**: IOMMU 关闭后，应优先使用 P2P。若必须调试死锁，`NCCL_P2P_DISABLE=1` 和 `NCCL_SHM_DISABLE=1` 是最有效的软隔离手段。

---

##4. 最终可运行脚本 (Final Train Script)此脚本基于 **PyTorch Native DDP** + **FP32 全精度**，适用于 MI50 32GB 双卡环境。
...
##5. 运维指南* **断点续训**:
1. 修改脚本中的 `--overwrite_output_dir` 为注释状态。
2. 添加/修改 `--resume_from_checkpoint True`。
3. 确保 `saves` 目录下有 `checkpoint-xxx` 文件夹。


* **监控**:
使用 `watch -n 1 rocm-smi` 查看 GPU 状态。正常运行时 GPU 利用率应有波动（加载数据与计算交替），显存占用应在 30GB+。
* **预期耗时**:
当前配置下，每 50 步约需 4.5 小时。跑完 5 个 Epoch 预计需 40 小时左右。
